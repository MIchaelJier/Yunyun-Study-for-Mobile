		...
		<yun-tab :tabs="tabs" :scrollTop="scrollTop" :bottomBarHeight="bottomBarHeight" @tabClick="tabClick" ref="tabs" >
			<template v-slot:0>
				...
				<view v-else :style="{paddingBottom:bottomBarHeight + 'px'}">
						...
						<!-- 加载更多 -->
						<uni-load-more :status="more_courses" iconType="circle" :iconSize="17" :contentText="contentText"></uni-load-more>
			...
			<template v-slot:1>
				...
				<view v-else :style="{paddingBottom:bottomBarHeight + 'px'}">
					...
					<!-- 加载更多 -->
					<uni-load-more :status="more_microSpecialty" iconType="circle" :iconSize="17" :contentText="contentText"></uni-load-more>
					<!-- <view class="bottom">已经到达底部了亲</view> -->
				</view>
			</template>
		</yun-tab>
